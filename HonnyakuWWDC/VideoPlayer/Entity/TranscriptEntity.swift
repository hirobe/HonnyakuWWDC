...
struct TranscriptEntity: Hashable, Codable {
    ...
    static var mock: TranscriptEntity = TranscriptEntity(language: "JA", paragraphs: [
        Paragraph(at: 0, sentences: Array<(Int,String)>([
            ...
            (4, "学習センターにタスクを追加して、関連するコードと、学習者が自分のコードでプロジェクトを拡張することを奨励するオプションの実験タスクを表示する方法を説明します。")]).map { at,text in Paragraph.Sentence(at: at, text: text) }),
        Paragraph(at: 5, sentences: Array<(Int,String)>([
